 ...
 */
export function initialize(callback: (data: Forecast) => void): void {
  // Save callback
  _callback = callback;
 ...
 */
export function loadFile(): Forecast {
  try {
    // Test if file exists
    if (existsSync(WEATHER_FILE)) {
      const data = readFileSync(WEATHER_FILE, "cbor") as Forecast;
      // data.description = "F";
      return data;
    }
  } catch (ex) {
 ...
 * @param data
 */
function setWeather(data: Forecast) {
  last = data;
  _callback(data);
}
